<script setup></script>

<template>
    <div class="">
        <footer class="bg-black h-40 px-10 w-screen flex justify-between items-center align-baseline" id="footer">
            <div class="flex justify-center align-middle font-serif md:text-lg text-sm text-slate-200">
                Made By - Jake Schildknecht
            </div>
            <div class="align-middle font-serif text-sm md:text-lg text-slate-200">
                <a> Email styledsoirees@outlook.com</a>
            </div>
        </footer>
    </div>
</template>
